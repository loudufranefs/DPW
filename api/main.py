...
from pageTemplate import PageTemplate#import abstract class that is used by the form template

class MainHandler(webapp2.RequestHandler):
    def get(self):
        #instance of form template
        p = FormTemplate()
        #array of input fields that will be generated by the input fields setter.
        p.input_fields = [['ip', 'text', 'IP Address'],['submit', 'submit']]
        
        #process information only if there is a GET
        if self.request.GET:
            im = IpModel() #model object class instance
            im.ip = self.request.GET['ip'] #get IP from URL
            im.callApi() #connect to the API
        
            iv = IpView() #view object class instance
            iv.view_array = im.model_array # data from model inserted into the dictionary to be used in the view object class
